...
#define EAGAIN 6        /* Resource temporarily unavailable 无数据，稍后重试/无资源，稍后重试 */
#define EALREADY 7      /* Connection already in progress */
#define EBADF 8         /* Bad file descriptor filds是个无效文件描述符*/
...
#define ECONNRESET 15   /* Connection reset 写入一个未连接的socket*/
#define EDEADLK 16      /* Resource deadlock would occur */
#define EDESTADDRREQ 17 /* Destination address required */
#define EDOM 18         /* Domain error */
#define EDQUOT 19       /* Reserved */

#define EEXIST 20       /* File exists  文件已存在*/
#define EFAULT 21       /* Bad address */
#define EFBIG 22        /* File too large 数据所有起始位置大于等于文件最大偏移值*/
...
#define EINTR 27        /* Interrupted function call 在函数执行期间被信号打断 */
#define EINVAL 28       /* Invalid argument 参数无效*/
#define EIO 29          /* Input/output error 发送I/O错误*/

#define EISCONN 30      /* Socket is connected */
#define EISDIR 31       /* Is a directory 目标文件名是个目录 */
#define ELOOP 32        /* Symbolic link loop 路径名存在链接循环*/
#define EMFILE 33       /* File descriptor value too large or too many open streams 进程无空闲文件描述符*/
...
#define ENOTDIR 57         /* Not a directory 在路径检索期间发送非目录项 */
...
#define ENXIO 63      /* No such device or address 设备文件不存在*/
#define EOPNOTSUPP 64 /* Operation not supported on socket */
#define EOVERFLOW 65  /* Value too large to be stored in data type 文件过大导致溢出*/
#define EOWNERDEAD 66 /* Previous owner died */
#define EPERM 67      /* Operation not permitted */
#define EPIPE 68      /* Broken pipe 向一个已经关闭的socket连接写入数据*/
...
#define ERANGE 72          /* Result too large or too small 超出文件流的传输容量*/
#define EROFS 73           /* Read-only file system 文件系统只读*/
#define ESPIPE 74          /* Invalid seek = fildes代表的是一个管道, FIFO, socket连接*/
...
#define ETXTBSY 79         /* Text file busy 文件正在执行中，无法进行读写操作*/

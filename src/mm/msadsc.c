...
void ret_msadsc_vadrandsz(msadsc_t **msavstart, u64_t* msar) {
    ...
    *msavstart = (msadsc_t*)(memory_management_struct.end_of_struct);
    ...
    INFOK("OS having %d 4k pagesize  = %d MB", TotalMem, (TotalMem * PAGE_4K_SIZE) / 1024 / 1024);
...
{
    INFOK("msadsc_t.md_f:_ux[%x],_my[%x],md_phyadrs:_alc[%x],_shd[%x],_swp[%x],_che[%x],_kmp[%x],_lck[%x],_dty[%x],_bsy[%x],_padrs[%#x]",
...
bool_t search_krloccupymsadsc_core()
{
    ...
    retschmnr = search_segment_occupymsadsc(msadstat, msanr, 0x100000, PAGE_4K_ALIGN(Virt_To_Phy(memory_management_struct.end_of_struct)));
    if (0 == retschmnr)
    {
        return FALSE;
    }
